...
For a full inventory with columns, constraints, and live row counts, see: [`docs/db-detailed-reference.md`](docs/db-detailed-reference.md)

| Table (physical) | App | Purpose |
|------------------|-----|---------|
| `authentication_superadmin` | authentication | Single Super‑Admin informational row (1:1 `auth_user`) |
| `authentication_adminprofile` | authentication | Admin user profile (avatar, bio, social links, etc.) |
| `settings_app_appsettings` | settings_app | App settings (app name, timezone, pagination, notifications) |
| `dashboard_activitylog` | dashboard | Append-only audit log of CRUD actions |
| `dashboard_admin` (Table1) | dashboard | Admin Management registry (username, role, hash, is_active) |
| `dashboard_user` (Table2) | dashboard | Domain users (demo table) |
| `dashboard_verified_artist` (Table3) | dashboard | Verified artists (demo table) |
| `dashboard_payment` (Table4) | dashboard | Payments (demo table) |
| `dashboard_artist_service` (Table5) | dashboard | Artist services (demo table) |
| `dashboard_artist_application` (Table6) | dashboard | Artist applications with workflow fields |
| `dashboard_artist_application_certificate` | dashboard | Uploaded certificates for applications |
| `dashboard_artist_availability` (Table7) | dashboard | Artist availability (demo table) |
| `dashboard_artist_calendar` (Table8) | dashboard | Artist calendar (demo table) |
| `dashboard_booking` (Table9) | dashboard | Bookings (FK optional → `dashboard_client`) |
| `dashboard_message` (Table10) | dashboard | Messages (FK optional → `dashboard_client`) |
| `dashboard_client` | dashboard | Client Portal accounts (UUID PK, phone/email unique) |
| `dashboard_client_log` | dashboard | Client Portal audit log (actions, performed_by) |
